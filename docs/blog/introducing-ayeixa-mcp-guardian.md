...
**Ayeixa MCP Guardian** (`@ayeixa/mcp-guardian`) is a runtime capability sandbox and boundary fence for Model Context Protocol servers and client tool invocations.
...
4. **GuardianSandbox**: Executes authorized tools inside isolated execution boundaries with bounded timeouts.
...
- **Sandbox Isolation**: Safe execution wrapper and timeout enforcement (`tests/sandbox.test.ts`).
...
- eBPF-level filesystem boundary enforcement is slated for future minor milestones.
